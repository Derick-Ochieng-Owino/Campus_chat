...
import '../profile/personal_details.dart';
...
class _CompleteProfilePageState extends State<CompleteProfilePage> {
  ...
  Future<void> _navigateToPersonalDetails() async {
    ...
    if (_selectedUniversity == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('Please complete all fields'), backgroundColor: colorScheme.error)
      );
      return;
    }
    ...
    try {
      ...
      if (units.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: const Text('Could not load units for selected profile. Please check selection.'), backgroundColor: colorScheme.error)
          );
        }
        return;
      }
      ...
      // 🛠️ DESIGN NOTE: When moving to PersonalDetailsPage, ensure that inside PersonalDetailsPage,
      // once the profile is completely committed to Firestore, you use:
      // Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const HomePage()), (route) => false);
      // This wipes the backward stack history so they can never land back on setup fields.
      Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => PersonalDetailsPage(academicData: academicData))
      );

    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error preparing data: $e'), backgroundColor: colorScheme.error)
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    // 🛠️ CRITICAL FIX: Replaced deprecated WillPopScope with modern PopScope to cleanly disable back navigation
    return PopScope(
      canPop: false,
      child: Scaffold(
        backgroundColor: theme.scaffoldBackgroundColor,
        body: LayoutBuilder( // 🛠️ CARD FIX: Aligned perfectly across your core auth layout components
                ...
                child: IntrinsicHeight(
                  child: Stack(
                    children: [
                      ...
                      // --- Main Card Layout Constraints Wrapper ---
                      Center( // 🛠️ VISUAL FIX: Brings card to absolute Y-axis centering alignment metrics
                        child: SafeArea(
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('We need a few details to set up your portal', style: theme.textTheme.bodyMedium!.copyWith(color: Colors.white70)),
